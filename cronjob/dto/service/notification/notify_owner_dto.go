...
type OwnerNotification struct {
	...
	NewVersionAvailable      bool   `json:"shouldUpdate,omitempty"`
}
...
func NewOwnerNotification(
	...
	newVersion string,
	newVersionAvailable bool) OwnerNotification {
...
func (dto OwnerNotification) GetSubject() string {
	if dto.NewVersionAvailable {
		return fmt.Sprintf(
			"ScanYourKube - Updating image %s to version %s of application %s",
			dto.ApplicationResourceName, dto.NewVersion,
			dto.ApplicationName,
		)

	}
	return fmt.Sprintf(
		"ScanYourKube - Couldn't update %s of application %s",
		dto.ApplicationResourceName,
		dto.ApplicationName,
	)
...
func (dto OwnerNotification) GetBody() string {
	vulnerabilityInfosBody := fmt.Sprintf(
		"<p>Our scan found the following vulnerability: %s</p><p>Details: %s</p>",
		dto.VulnerabilityName,
		dto.VulnerabilityDescription,
	)
	if dto.NewVersionAvailable {
		return fmt.Sprintf(
			"<html><body><h2>Update of image %s</h2>%s<p>"+
				"Be aware, that there might be problems with your application. "+
				"Please check if everything still works out.</p>"+
				"<p>Best regards, your Kubernetes administrator</p>"+
				"<p><i>Generated by ScanYourKube</i></p>",
			dto.ApplicationResourceName,
			vulnerabilityInfosBody,
		)
	} else {
		return fmt.Sprintf(
			"<html><body><h2>Update of image %s failed</h2>"+
